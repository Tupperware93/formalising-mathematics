...
begin
  split,
  intro hSiTF,
  exact ⟨(mem_sets_of_superset hSiTF) (inter_subset_left S T), (mem_sets_of_superset hSiTF) (inter_subset_right S T)⟩,

  intro hShT,
  exact inter_mem_sets hShT.1 hShT.2,
end
...
begin
  ...
  intros h x hS,
  rw le_def at h,
  exact h T (mem_principal_self T) hS,
  
  intros h x hT,
  rw mem_principal_sets at *,
  exact subset.trans h hT,
end
...
begin
  ...
  split,
  intros hF,
  specialize hF S ( mem_principal_self S),
  assumption,
  
  intros hF x hPS,
  rw mem_principal_sets at *,
  exact mem_sets_of_superset hF hPS,
end
...
{ sets := {X : set L | ∃ x : L, ∀ y, x ≤ y → y ∈ X},
  ...
  use max x y,
  intros h h',
  specialize hx (h) (le_trans  (le_max_left x y) h'),
  specialize hy (h) (le_trans  (le_max_right x y) h'),
  exact ⟨hx, hy⟩,
  end }
